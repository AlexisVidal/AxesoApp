...
[assembly: Dependency(typeof(IPlaceService))]
namespace AxesoConsumer.Interfaces
{
    ...
    {
        private const string GMapsApiKey = "AIzaSyAJ2Y_hbwcXftOfQbnUD_8ZNjpzwMQ3Dgs";
        ...
        public Task<PlacesAutoCompleteDto> GetAutoCompleteGoogleAddresses(string searchText)
        {
            return Task.Run(() =>
            {
                return new ApiServiceGoogle<PlacesAutoCompleteDto>().GetAsync(AUTOCOMPLETE_END_POINT,
        ...
        public Task<PlacesDetailDto> GetPlaceDetail(string place)
        {
            return Task.Run(() =>
            {
                return new ApiServiceGoogle<PlacesDetailDto>().GetAsync(PLACE_DETAIL_END_POINT,
        ...
        public Task<ReverseGeocodeDto> ReverseGeocode(string latlng)
        {
            return Task.Run(() =>
            {
                return new ApiServiceGoogle<ReverseGeocodeDto>().GetAsync(REVERSE_GEOCODE_END_POINT,
        ...
        public Task<RouteDirectionDto> GetRoute(string destination)
        {
            return Task.Run(() =>
            {
                return new ApiServiceGoogle<RouteDirectionDto>().GetAsync(ROUTE_DIRECTION_END_POINT,
                    new ApiParameter("origin", Store_LatLng),
